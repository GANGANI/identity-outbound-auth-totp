...
public class TOTPAuthenticator extends AbstractApplicationAuthenticator
    ...
    protected void initiateAuthenticationRequest(HttpServletRequest request,
        ...
        try {
            if (!TOTPUtil.isLocalUser(context)) {
                federatedUsername = FederatedAuthenticatorUtil.getLoggedInFederatedUser(context);
                // Check whether the federated user has a local association.
                if (federatedUsername != null  && StringUtils.isBlank(FederatedAuthenticatorUtil.
                        getLocalUsernameAssociatedWithFederatedUser(MultitenantUtils.
                        getTenantAwareUsername(federatedUsername), context))) {

                    if (log.isDebugEnabled()) {
                        log.debug("Initial login attempt of the federated user: " + federatedUsername);
                    }
                    log.debug("Initial login attempt of the federated user: " + federatedUsername);
                    ...
                    if (authenticatedUser != null) {
                        ...
                        federatedUsername = getFederatedUserName(authenticatedUser, context);
                        context.setProperty(TOTPAuthenticatorConstants.FEDERATED_USERNAME, federatedUsername);
                    }
                }
            }
            ...
            /*
            Not required to check the TOTP enable state for the initial login of the federated users as totp is not
            obviously enabled for the initial federated user login.
             */
            if (!isInitialFederatedUserLogin) {
    ...
    private String getFederatedUserName(AuthenticatedUser authenticatedUser, AuthenticationContext context)
        ...
        try {
            ...
            federatedUsername = UserSessionStore.getInstance().
                    getUserId(username, tenantId, userStoreDomain, idpId);
            if (StringUtils.isBlank(federatedUsername)) {
                federatedUsername = UUID.randomUUID().toString();
                UserSessionStore.getInstance().storeUserData(federatedUsername, username, tenantId,
                        userStoreDomain, idpId);
            }
            federatedUsername = federatedUsername + TOTPAuthenticatorConstants.TENANT_DOMAIN_SEPARATOR + tenantDomain;
        } catch (UserSessionException e) {
            throw new TOTPException("Error while getting the session data for user: " + username + " of " +
